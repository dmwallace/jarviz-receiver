...
app.get('/increaseVolume/:increment?',
	(res, req) => setVolumeDelta(res, req, 1))
app.get('/decreaseVolume/:increment?',
	(res, req) => setVolumeDelta(res, req, -1))
...
async function setForegroundWindow (name) {
	...
	var windowThreadProcessId = user32.GetWindowThreadProcessId(foregroundHWnd,
		null)
	...
	var attachThreadInput = user32.AttachThreadInput(windowThreadProcessId,
		currentThreadId, 0)
...
async function spawnChild ({ id, command, cwd, args }, res) {
	...
	if (child) {
		...
		if(args.join('').indexOf('microsoft-edge') > -1) {
			child.processName = 'microsoftedge.exe'
			setTimeout(() => {
...
async function killProcess () {
	...
	//isBusy = true
	await Promise.all(Object.entries(children).map(async ([pid, child]) => {
		return new Promise(async (resolve) => {
			if (child/* && child.pid*/) {
				...
				await fkill(child.pid, {force: true}).then(() => {
					...
					console.log(`\nCannot kill process by PID: ${child.pid}, attempting to kill by name: ${child.processName || child.spawnfile}`)
				...
				console.log("child.processName", child.processName);
				
				
				let args = `/im ${child.processName || child.spawnfile}`
				console.log("child.processName === 'microsoftedge.exe'", child.processName === 'microsoftedge.exe');
				
				if(child.processName === 'microsoftedge.exe') {
					args = `${args} /f`
...
async function sendTelnetCommand ({ cmd, host, port = 23, username, retries = 0, deferred = new Deferred() }) {
	...
	if (telnetBusy) {
		...
		telnetQueue.push(
			{ cmd, host, port, username, retries: retries + 1, deferred })
		...
		socket.on('data', function (data) {
			...
			switch (onDataCount) {
				...
				default:
					...
					socket.end()
					deferred.resolve(
						`Telnet command: ${cmd} sent successfully to ${host}:${port}`)
					telnetBusy = false
					popTelnetQueue()
					break
			}
	...
	socket.on('error', function (err) {
		...
		deferred.reject(new Error(
			`Failed to send Telnet command: ${cmd} to ${host}:${port}\n${err.message}`))
